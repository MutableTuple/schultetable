...
import wrongClickSound from "./assets/click.wav";
import finishSound from "./assets/click.wav";
...
function App() {
  ...
  const wrongClickSoundRef = useRef(new Audio(wrongClickSound));
  ...
  useEffect(() => {
    if (gameOver) {
      ...
      finishSoundRef.current.play(); // Play finish sound
  ...
  const playWrongClickSound = () => {
    wrongClickSoundRef.current.play();
  };
  ...
  const handleClick = (number) => {
    ...
    if (number === correctNumber) {
      ...
      playWrongClickSound(); // Play wrong click sound
      ...
      <div className="bg-white shadow-lg rounded-lg flex flex-col md:flex-row w-full max-w-4xl">
            ...
            >
              {numbers.map((number) => (
                <button
                  ...
                  onClick={() => {
                    handleClick(number);
                    playClickSound(); // Play click sound on each button click
                  }}
                >
                  {number}
                </button>
        ...
        <div className="w-full md:w-2/5 p-4 bg-gray-200 rounded-b-lg md:rounded-b-none md:rounded-r-lg">
          ...
          <button
            ...
            onClick={() => {
              shuffleNumbers();
              playClickSound(); // Play click sound on shuffle
            }}
          >
            Randomize Grid
          </button>
